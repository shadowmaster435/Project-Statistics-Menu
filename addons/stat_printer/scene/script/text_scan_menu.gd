...
func _process(delta):
	...
	StatCollectorGlobals.scanner_menu = self
	
func update_new_entry_child_position():
	var ch_count = vbox.get_child_count()
	vbox.move_child(add_entry, ch_count - 1)

const entry_scene = preload("res://addons/stat_printer/scene/text_scan_entry.tscn")
...
func load_save():
	for entry_key in StatCollectorGlobals.script_scanners:
		var entry_text = StatCollectorGlobals.script_scanners[entry_key]
...
func load_defaults():
	for entry_key in StatCollectorGlobals.script_scanner_defaults:
		var entry_text = StatCollectorGlobals.script_scanner_defaults[entry_key]
		var entry = entry_scene.instantiate(PackedScene.GEN_EDIT_STATE_DISABLED)
		vbox.add_child(entry)
		entry.name = str(randf())
		entry.menu_node = self
